...
public record UpdateChildProfileRequest(
	...
	@Schema(description = "변경할 어린이 별명", example = "영어왕민수", maxLength = 30)
	String nickname,

	@Positive(message = "나이는 1 이상이어야 합니다.")
	@Schema(description = "변경할 어린이 나이", example = "10", minimum = "1")
	Integer age,

	@Min(value = 1, message = "학년은 1 이상이어야 합니다.")
	@Max(value = 6, message = "학년은 6 이하여야 합니다.")
	@Schema(description = "변경할 초등학교 학년", example = "3", minimum = "1", maximum = "6")
	Integer grade,

	@Schema(description = "변경할 영어 학습 단계", example = "BEGINNER")
	ChildLevel level
) {
