...
전송한다. OpenAI 응답 대기 중 Tomcat 요청 스레드가 반환되는지, rate limit 대기용
...
| 용어                                | 의미                                                                                                                                                                                    |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Rate limiter                      | OpenAI 분당 요청·토큰 한도를 넘지 않도록 호출 시작 빈도를 제한하는 장치. rate token이 부족하면 bounded queue에서 refill을 기다린다. |
| Bounded queue                     | rate token을 즉시 얻지 못한 제출을 최대 64건, 10초 동안 FIFO로 보관한다. `CompletableFuture`만 보관해 request thread, 평가 executor thread, DB connection을 점유하지 않는다. queue full·timeout은 provider 호출 전 503이다. |
...
| Preparation limiter               | OpenAI 호출 전 DB 조회·제출 예약 같은 준비 구간의 동시 진입을 제한하는 장치. 현재는 구현하지 않았으며 OSIV 같은 자원 수명 문제를 제외한 뒤에도 준비 구간 병목이 재현될 때만 검토한다. |
| Recovery                          | 한 단계가 끝난 뒤 queue size와 Hikari pending이 10초 연속 0인지 확인하는 과정. 대기 항목과 DB connection이 정상 반환됐음을 확인한다. |
...
* OpenAI rate token이 부족한 요청은 최대 64건까지 비차단 FIFO queue가 흡수하며,
  queue full·timeout은 provider 호출 전 명시적인 503으로 끝나는가?

* 10·100·200·300명이 동시에 제출해도 사용자별 응답과 DB 대상이 섞이지
  않는가?

* 답안 제출 부하가 관계없는 일반 API까지 지연시키거나 실패시키지 않는가?
* 단계 종료 후 queue 항목과 DB connection이 모두 반환되는가?
...
답안 제출 burst
  → 제출 대상 조회·처리 예약
  → OpenAI rate token 즉시 획득 또는 비차단 FIFO queue 대기
  → OpenAI 평가 대기
  → 성공·실패 결과 기록
...
* 단계 사이에는 queue size와 Hikari pending이 10초 연속 0인지
  확인한다.
...
5. 각 단계가 끝나면 queue size와 Hikari pending이 10초 연속
...
| `answer_success`             | 최종 HTTP 200인 논리적 제출 수. 10단계에서는 전부 성공하고 100단계에서는 과반이어야 함                         |
...
성공 수가 동시 제출 수보다 작다는 사실만으로 실패라고 판정하지 않는다. rate token
잔여량과 queue 상태에 따라 즉시 시작 가능한 요청 수가 달라지므로 100단계는 과반
...
* OpenAI queue size는 항상 64 이하이다.
* 100단계는 과반 성공으로 짧은 burst를 대부분 흡수하고, 200·300단계에서는 raw
  정상 과부하 503이 관찰된다.
...
* 단계 종료 후 queue size와 Hikari pending이 모두 0으로 복구된다.
...
| OpenAI queue size 64 이하                 | 설정 불변식           | bounded queue가 설정 경계를 항상 지켜야 한다.                                                     |
| 100단계 과반 성공                         | burst 관찰            | rate limit과 queue가 짧은 burst를 대부분 흡수하는지 확인하되 100% 성공을 강제하지 않는다.                               |
| 200·300단계에서 정상 503 관찰             | 동작 계약             | queue 용량을 넘은 요청이 provider까지 전달되지 않고 명시적으로 거절되는지 증명한다.                                |
...
| 종료 후 queue·pending 10초 연속 0          | 자원 복구 계약         | queue 항목과 DB connection이 모두 반환돼야 한다. 10초는 일시적인 완료 순서 차이를 흡수하는 안정화 구간이다.                         |
...
두 PowerShell 스크립트의 `-AssessmentQueueCapacity`,
...
queue size와 Hikari pending이 10초 연속 0이 될 때까지 기다리며,
...
상태에서 서로 다른 `Stages`, queue, 재시도 설정으로 `run-stages.ps1`을 여러 번
...
`hikaricp.connections.pending=0`을 확인한다. 실제 OpenAI는 rate token을 획득한
요청만 호출되므로 네 단계 합계 최대 610회의 과금 호출이 발생할 수
있다.
