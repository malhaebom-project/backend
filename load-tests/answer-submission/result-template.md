...
| 동시 제출 | 최대 active | 최대 queue | queue full | queue timeout | queue cancelled | 종료 active·queue·Hikari pending |
| ---: | ---: | ---: | ---: | ---: | ---: | :---: |
| 10 |  |  |  |  |  |  |
| 100 |  |  |  |  |  |  |
| 200 |  |  |  |  |  |  |
...
- active 제한 이하 단계는 전부 성공:
- 100단계에서 성공이 과반으로 queue 경계를 대부분 흡수:
- 수용량 96 밖의 200/300단계에서 raw 정상 503 관찰:
- raw 503 p95 12초 이내(최대 queue wait 10초 + 응답 여유 2초):
- OpenAI active 32 이하, queue size 64 이하:
